...
class ImageSequenceAerialView extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            imagesBaseUrl: "../../../images/HouseStyle/102GIF/",    //圖片所在的目錄
        ...
        if (this.canvasDOMElement) {
            ...
            if (this.canvasImageArray.length === 0) {
                ...
                for (let i = this.state.imageNameStartNumber; i < this.state.totalImagesNumber; i++) {
                    let img = new Image();
                    img.src = require('../../../images/HouseStyle/102GIF/' + i + this.state.imagesFormat);
                    // img.src = require("../../../images/HouseStyle/102GIF/0.jpg");
        ...
        if (this.imageArray.length === 0) {
            ...
            for (let i = this.state.imageNameStartNumber; i < this.state.totalImagesNumber; i++) {
                ...
                this.imageArray.push(
                    <img ref={self => this.DOMArray.push(self)} className={'image ' + "image" + i + " hidden"} src={require('../../../images/HouseStyle/102GIF/' + i + this.state.imagesFormat)} onLoad={() => this.handleEachImageLoaded()} key={"image" + currentTime + i} alt="" />
                );
        ...
        return (
            <div className="ImageSequenceAerialViewContainer" onTouchMove={(event) => this.handleTouchMove(event)} onTouchStart={(event) => this.handleTouchStart(event)} onTouchEnd={() => this.handleTouchEnd()}>
                ...
                <div className={(this.state.blurPreviewImageIsShowing) ? ("previewImg") : ("previewImg hidden")}>
                    <img src={require(('../../../images/HouseStyle/102GIF/' + this.state.imageNameStartNumber + this.state.imagesFormat))} alt="" />
                </div>
